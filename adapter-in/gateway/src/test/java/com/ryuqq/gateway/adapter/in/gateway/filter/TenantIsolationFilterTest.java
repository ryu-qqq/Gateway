...
class TenantIsolationFilterTest {
    ...
    @Nested
    @DisplayName("헤더 전달 테스트")
    class HeaderPropagationTest {

        @Test
        @DisplayName("X-Tenant-Id 헤더를 Request와 Response에 추가")
        void shouldAddTenantIdHeaderToRequestAndResponse() {
            // given
            MockServerHttpRequest request = MockServerHttpRequest.get("/api/test").build();
            MockServerWebExchange exchange = MockServerWebExchange.from(request);
            exchange.getAttributes().put("userId", "user-123");
        ...
        @Test
        @DisplayName("organizationId가 있으면 X-Organization-Id 헤더 추가")
        void shouldAddOrganizationIdHeaderWhenPresent() {
            // given
            MockServerHttpRequest request = MockServerHttpRequest.get("/api/test").build();
            MockServerWebExchange exchange = MockServerWebExchange.from(request);
            exchange.getAttributes().put("userId", "user-123");
            exchange.getAttributes().put("tenantId", "tenant-123");
            exchange.getAttributes().put("organizationId", "org-789");
            exchange.getAttributes().put("roles", Set.of("ROLE_USER"));
        ...
        @Test
        @DisplayName("organizationId가 없으면 X-Organization-Id 헤더 추가하지 않음")
        void shouldNotAddOrganizationIdHeaderWhenAbsent() {
            // given
            MockServerHttpRequest request = MockServerHttpRequest.get("/api/test").build();
            MockServerWebExchange exchange = MockServerWebExchange.from(request);
            exchange.getAttributes().put("userId", "user-123");
            exchange.getAttributes().put("tenantId", "tenant-123");
            // organizationId 설정하지 않음
            exchange.getAttributes().put("roles", Set.of("ROLE_USER"));
            ...
            // then - X-Organization-Id 헤더 없음
            assertThat(exchange.getResponse().getHeaders().getFirst("X-Organization-Id")).isNull();
        }
    }
