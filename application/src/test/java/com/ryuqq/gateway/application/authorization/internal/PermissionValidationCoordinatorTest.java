...
/**
 * PermissionValidationCoordinator 단위 테스트
 *
 * <p>Permission Spec + Permission Hash 기반 권한 검증 로직 테스트
 *
 * @author development-team
 * @since 1.0.0
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("PermissionValidationCoordinator 단위 테스트")
class PermissionValidationCoordinatorTest {
    ...
    @InjectMocks private PermissionValidationCoordinator permissionValidationCoordinator;
    ...
    class PublicEndpointHandling {
        ...
        void allowPublicEndpointWithoutPermissionCheck() {
            ...
            // when
            Mono<ValidatePermissionResponse> result =
                    permissionValidationCoordinator.validate(command);
            ...
            then(permissionHashCoordinator).shouldHaveNoInteractions();
        }
    }
        ...
        void allowWhenUserHasAllRequiredPermissions() {
            ...
            given(
                            permissionHashCoordinator.findByTenantAndUser(
                                    TENANT_ID, USER_ID, PERMISSION_HASH))
