...
resource "aws_kms_key" "logs" {
  description             = "KMS key for Gateway CloudWatch logs encryption (${var.environment})"
...
module "gateway_logs" {
  ...
  retention_in_days = 14  # Stage: 더 짧은 보관 기간
...
resource "aws_security_group" "alb" {
  ...
  description = "Security group for ALB (${var.environment})"
  vpc_id      = local.vpc_id
...
module "ecs_security_group" {
  ...
  description = "Security group for gateway ECS tasks (${var.environment})"
  vpc_id      = local.vpc_id
...
module "ecs_service" {
  ...
  container_environment = [
    ...
    # Legacy API 라우팅 (Stage Cloud Map DNS)
    { name = "LEGACY_WEB_URI", value = "http://setof-commerce-legacy-api-stage.connectly.local:8088" },
    { name = "LEGACY_ADMIN_URI", value = "http://setof-commerce-legacy-admin-stage.connectly.local:8089" },
    # Sentry/Observability
    { name = "SERVICE_NAME", value = "${var.project_name}-${var.environment}" },
