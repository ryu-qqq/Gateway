...
# ===============================================
# Stage RDS와 ElastiCache에 안전하게 접근하기 위한 포트 포워딩 스크립트
# ===============================================
...
# Stage ElastiCache 엔드포인트 찾기 (SSM Parameter Store 사용)
...
if [ -z "$REDIS_ENDPOINT" ] || [ "$REDIS_ENDPOINT" == "None" ]; then
    echo -e "${YELLOW}⚠️  Stage ElastiCache 엔드포인트를 찾을 수 없습니다. Redis 포트 포워딩을 건너뜁니다.${NC}"
    REDIS_ENDPOINT=""
else
...
# Stage Redis 포트 포워딩 (16382 포트 사용 - Prod는 16381)
if [ -n "$REDIS_ENDPOINT" ]; then
    echo ""
    echo "Stage Redis 포트 포워딩 시작 (localhost:16382 -> ${REDIS_ENDPOINT}:6379)..."
    aws ssm start-session \
        --region $AWS_REGION \
        --target $BASTION_INSTANCE_ID \
        --document-name AWS-StartPortForwardingSessionToRemoteHost \
        --parameters "{\"host\":[\"${REDIS_ENDPOINT}\"],\"portNumber\":[\"6379\"],\"localPortNumber\":[\"16382\"]}" \
        > /dev/null 2>&1 &
    ...
    echo ""
    echo -e "${BLUE}   ┌─────────────────────────────────────────────────────────────┐${NC}"
    echo -e "${BLUE}   │  Stage Redis 접속 명령어:                                     │${NC}"
    echo -e "${BLUE}   │  redis-cli -h 127.0.0.1 -p 16382                            │${NC}"
    echo -e "${BLUE}   └─────────────────────────────────────────────────────────────┘${NC}"
fi
...
echo -e "   - Stage Redis: localhost:16382"
echo -e "   - Prod RDS:    localhost:13307 (aws-port-forward.sh 사용)"
echo -e "   - Prod Redis:  localhost:16381 (aws-port-forward.sh 사용)"
echo ""
